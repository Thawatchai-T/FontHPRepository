...
{
    public class RequestComHiringMap : ClassMap<RequestComHiringDomain>
    {

        public RequestComHiringMap()
        {
            Table("Request_Com_Hiring");
            LazyLoad();
            CompositeId().KeyProperty(x => x.RequestNo, "Request_No")
                         .KeyProperty(x => x.SeqNo, "Seq_No");
            Map(x => x.Saleman).Column("SaleMan").Length(14);
